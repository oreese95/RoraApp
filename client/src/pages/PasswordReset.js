  ...
  function onFinish(values) {
    users.map((user) => {
      if (user._id == userid) {
        ...
        emailjs.send(globalVar.Gmail_SRV, globalVar.PW_Reset_Confirm, {
      ...
      <div className="mt-4 d-flex justify-content-center">
        <div className='fpEmail'>
          <Form layout='vertical' onFinish={onFinish}>
            ...
            <Form.Item name='password2' label='Confirm New Password' rules={[{ required: true }]}>
            ...
            />
            {valid ? <button className='logbutton mt-2 mb-3'>Submit</button> : ""}
          </Form>
        </div>
      </div>
    </div>
